...
namespace SpicesTest {
	...
	*/
	...
	/**
	* @brief Testing Spices::MemoryPool Stress.
	*/
	TEST_F(MemoryPool_test, StressTesting) {

		SPICESTEST_PROFILE_FUNCTION();
		 
		constexpr int nThreads   = 4;
		constexpr int nCounts_0  = 0;   // 900M
		constexpr int nCounts_1  = 0;      // 900M
		constexpr int nCounts_2  = 1;      // 900M
		constexpr int nCounts_3  = 0;        // 900M
		...
		for (int i = 0; i < nThreads; i++)
		{
			std::thread t([&]() {
				...
				for(;;)
				{
					static  _declspec(thread) int index = 0;
					index++;

					std::vector<void*> objects;
					objects.resize(nCounts_0 + nCounts_1 + nCounts_2 + nCounts_3);

					// 8 - 1024 B
					for (int i = 0; i < nCounts_0; i++)
					{
						float rand   = std::rand() / float(RAND_MAX);
						size_t bytes = rand * (1024 - 8)  + 8 + 1;

						objects[i]   = Spices::MemoryPool::Alloc(bytes);
					}

					// 1 - 256 KB
					for (int i = nCounts_0; i < nCounts_0 + nCounts_1; i++)
					{
						float rand   = std::rand() / float(RAND_MAX);
						size_t bytes = rand * (256 * 1024 - 1024) + 1024 + 1;

						objects[i]   = Spices::MemoryPool::Alloc(bytes);
					}

					// 256 - 4 * 256 KB
					for (int i = nCounts_0 + nCounts_1; i < nCounts_0 + nCounts_1 + nCounts_2; i++)
					{
						float rand   = std::rand() / float(RAND_MAX);
						size_t bytes = rand * (4 * 256 * 1024 - 256 * 1024) + 256 * 1024 + 1;

						objects[i]   = Spices::MemoryPool::Alloc(3 * 256 * 1024);
					}

					// 1 - 8 M
					for (int i = nCounts_0 + nCounts_1 + nCounts_2; i < nCounts_0 + nCounts_1 + nCounts_2 + nCounts_3; i++)
					{
						float rand = std::rand() / float(RAND_MAX);
						size_t bytes = rand * (8 * 1024 * 1024 - 1024 * 1024) + 1024 * 1024 + 1;

						objects[i] = Spices::MemoryPool::Alloc(bytes);
					}

					for (int i = 0; i < nCounts_0 + nCounts_1 + nCounts_2 + nCounts_3; i++)
					{
						Spices::MemoryPool::Free(objects[i]);
					}
