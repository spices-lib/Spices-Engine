...
namespace nv { namespace perf { namespace sampler {
    ...
    class PeriodicSamplerTimeHistoryD3D12
    {
    private:
        ...
        mini_trace::MiniTracerD3D12 m_tracer;
    ...
    public:
        PeriodicSamplerTimeHistoryD3D12()
            : m_maxTriggerLatency()
            , m_isFirstFrame(true)
            , m_status(SamplerStatus::Uninitialized)
        {
        }
        PeriodicSamplerTimeHistoryD3D12(const PeriodicSamplerTimeHistoryD3D12& sampler) = delete;
        PeriodicSamplerTimeHistoryD3D12(PeriodicSamplerTimeHistoryD3D12&& sampler)
        ...
        PeriodicSamplerTimeHistoryD3D12& operator=(const PeriodicSamplerTimeHistoryD3D12& sampler) = delete;
        PeriodicSamplerTimeHistoryD3D12& operator=(PeriodicSamplerTimeHistoryD3D12&& sampler)
        {
            Reset();
            m_periodicSamplerGpu = std::move(sampler.m_periodicSamplerGpu);
            m_counterData = std::move(sampler.m_counterData);
        ...
        bool Initialize(ID3D12Device* pDevice)
        {
            ...
            const size_t sliIndex = 0; // TODO: support sli
            const size_t deviceIndex = D3D12GetNvperfDeviceIndex(pDevice, sliIndex);
            if (deviceIndex == (size_t)~0)
            {
                NV_PERF_LOG_ERR(20, "D3D12GetNvperfDeviceIndex failed\n");
                return false;
            }
            ...
            success = m_tracer.Initialize(pDevice);
            if (!success)
            {
                return false;
            }
        ...
        bool BeginSession(ID3D12CommandQueue* pCommandQueue, uint32_t samplingIntervalInNanoSeconds, uint32_t maxDecodeLatencyInNanoSeconds, size_t maxFrameLatency)
        {
            ...
            success = m_tracer.BeginSession(pCommandQueue, maxFrameLatency);
            if (!success)
            {
                return false;
            }
        ...
        std::vector<FrameDelimiter> GetFrameDelimiters()
        {
            ...
            while (true)
            {
                ...
                mini_trace::MiniTracerD3D12::FrameData frameData{};
...
"3) call DecodeCounters() more frequently\n");
    ...
    class PeriodicSamplerOneShotD3D12
    {
    public:
        enum class SamplerStatus
        {
        ...
        PeriodicSamplerOneShotD3D12(const PeriodicSamplerOneShotD3D12& sampler) = delete;
        PeriodicSamplerOneShotD3D12(PeriodicSamplerOneShotD3D12&& sampler) = default;
        ~PeriodicSamplerOneShotD3D12()
        {
            Reset();
        }
        PeriodicSamplerOneShotD3D12& operator=(const PeriodicSamplerOneShotD3D12& sampler) = delete;
        PeriodicSamplerOneShotD3D12& operator=(PeriodicSamplerOneShotD3D12&& sampler) = default;
        ...
        bool Initialize(ID3D12Device* pDevice, size_t samplingIntervalNs, size_t maxDurationPerFrameNs, size_t numFramesToSample,
                        std::function<void(const char*)> onStopSampling, const char* pCounterConfigFileName = "", const char* pCounterDataPrefixFileName = ""
        )
        {
            if (m_status != SamplerStatus::Uninitialized)
            {
            ...
            if (!D3D12IsNvidiaDevice(pDevice))
            {
                NV_PERF_LOG_ERR(10, "Not running on an NVIDIA device\n");
                return false;
            }

            const size_t deviceIndex = D3D12GetNvperfDeviceIndex(pDevice);
            if (deviceIndex == (size_t)~0)
            {
                NV_PERF_LOG_ERR(20, "D3D12GetNvperfDeviceIndex failed\n");
                return false;
            ...
            auto isNullOrEmpty = [](const char* pStr) -> bool {
                return !pStr || !pStr[0];
            };
            ...
            if (m_counterConfiguration.numPasses != 1)
            {
        ...
        bool OnFrameEnd()
        {
            if (m_inCollection && m_status == SamplerStatus::Sampling)
            {
                ...
                if (m_numFramesSampled == m_numFramesToSample)
                {
                    ...
                    {
                        std::string counterDataFileName = m_outputDirectory + std::string(1, NV_PERF_PATH_SEPARATOR) + "counterData.bin";
    ...
    private:
        bool InitializeCounterData()
        {
            ...
            bool success = GpuPeriodicSamplerCreateCounterData(
                    ...
                    (uint32_t)maxSamples, NVPW_PERIODIC_SAMPLER_COUNTER_DATA_APPEND_MODE_LINEAR, m_counterData
            );
            if (!success)
            {
