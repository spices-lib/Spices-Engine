	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				// profiling output.
				//if (Spices::Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				//{
				//	SPICES_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				//}
				InternalEndSession();
			}
			...
			else
			{
				//if (Spices::Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				//{
				//	SPICES_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				//}
			}
		}
...
#define SPICES_FUNC_SIG __PRETTY_FUNCTION__
#elif defined(__DMC__) && (__DMC__ >= 0x810)
#define SPICES_FUNC_SIG __PRETTY_FUNCTION__
#elif (defined(__FUNCSIG__) || (_MSC_VER))
#define SPICES_FUNC_SIG __FUNCSIG__
#elif (defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 600)) || (defined(__IBMCPP__) && (__IBMCPP__ >= 500))
#define SPICES_FUNC_SIG __FUNCTION__
#elif defined(__BORLANDC__) && (__BORLANDC__ >= 0x550)
#define SPICES_FUNC_SIG __FUNC__
...
#define SPICESTEST_PROFILE_SCOPE_LINE2(name, line)         constexpr auto fixedName##line = ::SpicesTest::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
