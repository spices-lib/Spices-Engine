...
namespace nv { namespace perf { namespace hud {
    ...
    private:
        ...
        public:
            ...
            size_t AddMetric(const std::string& metric, const NVPW_MetricEvalRequest* pRequest_ = nullptr)
            {
                ...
                if (!m_configBuilder.AddMetrics(pRequest, 1, KeepInstances))
                {
                    return InvalidIndex;
                }
            ...
            bool GenerateCounterConfiguration(CounterConfiguration& counterConfiguration, bool validateGeneratedConfiguration = true)
            {
                if (!CreateConfiguration(m_configBuilder, counterConfiguration))
                {
                    return false;
                }
                if (validateGeneratedConfiguration && counterConfiguration.numPasses != 1u)
                {
    ...
    public:
        ...
        bool Initialize(double samplingIntervalInSeconds, double plotTimeWidthInSeconds, bool validateGeneratedConfiguration = true)
        {
            if (IsInitialized())
            {
                NV_PERF_LOG_ERR(20, "Already initialized\n");
            ...
            if (!counterConfigBuilder.GenerateCounterConfiguration(m_counterConfiguration, validateGeneratedConfiguration))
            {
                return false;
            }
