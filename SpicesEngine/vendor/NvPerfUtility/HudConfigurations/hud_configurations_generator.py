...
def gen_depfile(target_file_path, yaml_file_names, buildroot):
    ...
    for file_name in yaml_file_names:
        depfile_contents.append('\t' + file_name + ' \\')
...
# yaml_file_names  : list of hud configuration yaml files that are embedded in the target header file
...
namespace nv {{ namespace perf {{ namespace hud {{ namespace {} {{ namespace HudConfigurations {{
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yamlPaths', default=[], nargs='+', required=True, help="Yaml HudConfigurations paths")
    ...
    cpp_file_name = 'NvPerfHudConfigurations{}.h'.format(chip.upper())
