...
namespace SpicesTest {
	...
	*/
	TEST_F(VulkanRenderBackend_test, Initialize) {
		...
		EXPECT_EQ(m_RenderBackend->GetState().m_SwapChainImages.size(),            Spices::MaxFrameInFlight   );    /* @see Spices::VkImage              */
		EXPECT_NE(m_RenderBackend->GetState().m_SwapChainImageSamplers.data(),     nullptr                    );    /* @see Spices::VkSampler            */
		EXPECT_EQ(m_RenderBackend->GetState().m_SwapChainImageSamplers.size(),     Spices::MaxFrameInFlight   );    /* @see Spices::VkSampler            */
		EXPECT_NE(m_RenderBackend->GetState().m_SwapChainImageViews.data(),        nullptr                    );    /* @see Spices::VkImageView          */ 
		EXPECT_EQ(m_RenderBackend->GetState().m_SwapChainImageViews.size(),        Spices:: MaxFrameInFlight  );    /* @see Spices::VkImageView          */
		...
		EXPECT_EQ(m_RenderBackend->GetState().m_GraphicImageSemaphore.size(),      Spices::MaxFrameInFlight   );    /* @see Spices::VkSemaphore          */
		EXPECT_NE(m_RenderBackend->GetState().m_GraphicQueueSemaphore.data(),      nullptr                    );    /* @see Spices::VkSemaphore          */
		EXPECT_EQ(m_RenderBackend->GetState().m_GraphicQueueSemaphore.size(),      Spices::MaxFrameInFlight   );    /* @see Spices::VkSemaphore          */
		EXPECT_NE(m_RenderBackend->GetState().m_GraphicFence.data(),               nullptr                    );    /* @see Spices::VkFence              */
		EXPECT_EQ(m_RenderBackend->GetState().m_GraphicFence.size(),               Spices::MaxFrameInFlight   );    /* @see Spices::VkFence              */  
		EXPECT_NE(m_RenderBackend->GetState().m_GraphicCommandBuffer.data(),       nullptr                    );    /* @see Spices::VkCommandBuffer      */
		EXPECT_EQ(m_RenderBackend->GetState().m_GraphicCommandBuffer.size(),       Spices::MaxFrameInFlight   );    /* @see Spices::VkCommandBuffer      */
		...
		EXPECT_EQ(m_RenderBackend->GetState().m_ComputeQueueSemaphore.size(),      Spices::MaxFrameInFlight   );    /* @see Spices::VkSemaphore          */
		EXPECT_NE(m_RenderBackend->GetState().m_ComputeFence.data(),               nullptr                    );    /* @see Spices::VkFence              */
		EXPECT_EQ(m_RenderBackend->GetState().m_ComputeFence.size(),               Spices::MaxFrameInFlight   );    /* @see Spices::VkFence              */
		EXPECT_NE(m_RenderBackend->GetState().m_ComputeCommandBuffer.data(),       nullptr                    );    /* @see Spices::VkCommandBuffer      */
		EXPECT_EQ(m_RenderBackend->GetState().m_ComputeCommandBuffer.size(),       Spices::MaxFrameInFlight   );    /* @see Spices::VkCommandBuffer      */
